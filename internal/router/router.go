...
func AuthRoutes(router *gin.Engine, cont *container.Container) {
	...
	auth.POST("/logout", middleware.AuthMiddleware(cont.AuthService), cont.AuthController.Logout)
}

func StaffSkillRoutes(router *gin.Engine, cont *container.Container) {
	group := router.Group("/api/staff-skills")
	group.POST("/", middleware.AuthMiddleware(cont.AuthService), middleware.RequireRole("admin"), cont.StaffSkillController.AssignSkill)
	...
	group.DELETE("/:id", middleware.AuthMiddleware(cont.AuthService), middleware.RequireRole("admin"), cont.StaffSkillController.UnassignSkill)
}

func TreatmentRoutes(router *gin.Engine, cont *container.Container) {
	treatment := router.Group("/api/treatment")

	treatment.POST("/", middleware.AuthMiddleware(cont.AuthService), middleware.RequireRole("admin"), cont.TreatmentController.CreateTreatment)
	treatment.GET("/list", cont.TreatmentController.ListTreatments)
	treatment.GET("/:id", cont.TreatmentController.GetTreatment)
	treatment.PUT("/:id", middleware.AuthMiddleware(cont.AuthService), cont.TreatmentController.UpdateTreatment)
	treatment.DELETE("/:id", middleware.AuthMiddleware(cont.AuthService), middleware.RequireRole("admin"), cont.TreatmentController.DeleteTreatment)
}

func UserRoutes(router *gin.Engine, cont *container.Container) {
	user := router.Group("/api/user")

	user.POST("/register-staff", middleware.AuthMiddleware(cont.AuthService), middleware.RequireRole("admin"), cont.UserController.RegisterStaff)
	user.POST("/verify", cont.UserController.VerifyUser)
	user.GET("/list", cont.UserController.ListUsers)
	user.PUT("/:id", middleware.AuthMiddleware(cont.AuthService), cont.UserController.UpdateUser)
	user.DELETE("/:id", middleware.AuthMiddleware(cont.AuthService), middleware.RequireRole("admin"), cont.UserController.DeleteUser)
	user.POST("/:id/photo", middleware.AuthMiddleware(cont.AuthService), cont.UserController.UploadPhoto)
}
